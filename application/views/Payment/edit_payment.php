	...
	<div class="col-sm-6  " style="margin: 0 auto; ">
		...
		<div class="card ">
			...
			<div class="card-body ">
					...
					<div class="col-sm-9">
						<select name="t_id" class="form-control" >
							...
							<?php foreach($tenantNames as $tname): ?>
								<option value="<?=$tname->id; ?>" <?= $tname->id==$data->t_id?"selected":"" ?>><?= $tname->name;?></option>
							<?php  endforeach; ?>
						</select>
					...
					<div class="col-sm-9">
						<select name="r_id" class="form-control" >
							...
							<?php foreach($rents as $rent): ?>
								<option value="<?=$rent->id; ?>" <?= $rent->id==$data->r_id?"selected":"" ?>><?= $rent->month;?> - <?= $rent->year;?> - <?= $rent->rent + $rent->elc_bill + $rent->gass_bill;?></option>
							<?php  endforeach; ?>
						</select>
					...
					<div class="col-sm-9">
						<select name="house_no" class="form-control" >
							...
							<?php foreach($houses as $house): ?>
								<option value="<?= $house->id; ?>" <?= $house->id==$data->house_no?"selected":"" ?>><?= $house->house; ?></option>
							<?php endforeach; ?>
						</select>
				...
				<div class="form-group row">
					<label for="cat_name" class="col-sm-3"> Payment </label>
					<div class="col-sm-9">
